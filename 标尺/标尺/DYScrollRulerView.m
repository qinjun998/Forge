...
//            NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:UIColorFromRGB(0xf7f317)};   //文字和标尺刻度的颜色
...
-(instancetype)initWithFrame:(CGRect)frame theMinValue:(float)minValue theMaxValue:(float)maxValue theStep:(float)step theUnit:(NSString *)unit theNum:(NSInteger)betweenNum{
    ...
    if (self) {
        ...
        _bgColor    = [UIColor whiteColor];  //无作用 被后来的设置覆盖

        _triangleColor          = [UIColor orangeColor]; //默认橙色
        self.backgroundColor    = [UIColor whiteColor];  //大控件的颜色
